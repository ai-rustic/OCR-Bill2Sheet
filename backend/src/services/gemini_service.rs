...
impl GeminiService {
    ...
    pub fn new(config: Option<GeminiConfig>) -> Result<Self, GeminiError> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| {
                GeminiError::NetworkError(format!("Failed to create HTTP client: {}", e))
            })?;
    ...
    pub async fn extract_bill_data(
        &self,
        image_data: &[u8],
    ) -> Result<Vec<GeminiResponse>, GeminiError> {
        let start_time = Instant::now();
        info!(
            "Starting Gemini bill data extraction for image of {} bytes",
            image_data.len()
        );
        ...
        debug!(
            "Successfully encoded image to base64, length: {}",
            encoded_image.len()
        );
        ...
        match self.send_request_with_retry(&request).await {
            Ok(responses) => {
                ...
                info!(
                    "Successfully extracted {} bill candidate(s) from Gemini API in {:?}",
                    responses.len(),
                    duration
                );
                ...
                if let Some(first) = responses.first() {
                    debug!(
                        "First extracted candidate contains: form_no={:?}, invoice_no={:?}",
                        first.form_no, first.invoice_no
                    );
                }
            ...
            Err(e) => {
                let duration = start_time.elapsed();
                error!(
                    "Failed to extract bill data from Gemini API after {:?}: {}",
                    duration, e
                );
                Err(e)
            }
            ...
            match self.send_gemini_request(request).await {
                ...
                Err(GeminiError::RateLimitExceeded { retry_after }) => {
                    if attempt < self.config.max_retries {
                        ...
                        warn!(
                            "Gemini API rate limit exceeded on attempt {}. Retrying in {}ms",
                            attempt + 1,
                            delay_ms
                        );
                        ...
                        error!(
                            "Gemini API rate limit exceeded. Max retries ({}) reached",
                ...
                Err(e) => {
                    error!(
                        "Gemini API request failed on attempt {}: {}",
                        attempt + 1,
                        e
                    );
                    last_error = Some(e);
    ...
    async fn send_gemini_request(
        ...
        let payload = json!({
            ...
            "generationConfig": {
                ...
                "responseSchema": {
                    "type": "array",
                    "description": "Danh sách các mục hóa đơn được trích xuất",
                    "items": {
                        "type": "object",
                        "properties": {
                            "form_no": {
        ...
        debug!(
            "Sending HTTP request to Gemini API with timeout of {} seconds",
        ...
        if status == 429 {
            ...
            warn!(
                "Gemini API rate limit exceeded (429). Retry after: {:?}",
                retry_after
            );
        ...
        if status == 401 || status == 403 {
            error!(
                "Gemini API authentication failed ({}). Check API key",
                status
            );
    ...
    async fn parse_gemini_response(
        ...
        let candidates = response["candidates"].as_array().ok_or_else(|| {
            GeminiError::InvalidResponseFormat("Missing candidates array".to_string())
        })?;

        if candidates.is_empty() {
            return Err(GeminiError::InvalidResponseFormat(
        ...
        let parts = candidates[0]["content"]["parts"]
            .as_array()
            .ok_or_else(|| {
                GeminiError::InvalidResponseFormat("Missing content parts array".to_string())
            })?;
        ...
        let content = parts
            ...
            .ok_or_else(|| {
                GeminiError::InvalidResponseFormat("Missing text content".to_string())
            })?;
        ...
        debug!(
            "Parsing structured JSON response from Gemini: {}",
            cleaned_content
        );

        match serde_json::from_str::<Vec<GeminiResponse>>(&cleaned_content) {
            Ok(responses) => {
                if responses.is_empty() {
                    error!(
                        "Structured JSON response parsed to an empty array: {}",
                        cleaned_content
                    );
                    Err(GeminiError::InvalidResponseFormat(
                        "Parsed array is empty".to_string(),
                    ))
                } else {
                    if let Some(first) = responses.first() {
                        debug!(
                            "Successfully parsed structured response with form_no={:?}, invoice_no={:?}",
                            first.form_no, first.invoice_no
                        );
                    }
                    Ok(responses)
            ...
            Err(primary_err) => {
                warn!(
                    "Failed to parse Gemini response as array: {}. Attempting single-object fallback. Response: {}",
                    primary_err, cleaned_content
                );

                match serde_json::from_str::<GeminiResponse>(&cleaned_content) {
                    Ok(single) => {
                        warn!(
                            "Gemini response returned a single object; wrapping in array for backwards compatibility"
                        );
                        Ok(vec![single])
                    }
                    Err(e) => {
                        error!(
                            "Failed to parse structured JSON response: {}. Response: {}",
                            e, cleaned_content
                        );
                        Err(GeminiError::InvalidResponseFormat(format!(
                            "Failed to parse structured JSON response: {}. Response: {}",
                            e, cleaned_content
                        )))
                    }
    ...
    pub async fn test_connection(&self) -> Result<(), GeminiError> {
        ...
        let test_image = vec![
            ...
            0xD7, 0x63, 0xF8, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
            0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82,
        ];

        let encoded = self.encode_image(&test_image)?;
        let test_request = GeminiRequest::new(encoded, "Describe this image briefly.".to_string());
...
mod tests {
    ...
    fn test_clean_json_response() {
        ...
        assert_eq!(
            service.clean_json_response(markdown_json),
            "{\"test\": \"value\"}"
        );
        ...
        let simple_markdown = "```\n{\"test\": \"value\"}\n```";
        assert_eq!(
            service.clean_json_response(simple_markdown),
            "{\"test\": \"value\"}"
        );
        ...
        assert_eq!(
            service.clean_json_response(plain_json),
            "{\"test\": \"value\"}"
        );
        ...
        let whitespace_json = "  \n  {\"test\": \"value\"}  \n  ";
        assert_eq!(
            service.clean_json_response(whitespace_json),
            "{\"test\": \"value\"}"
        );
    }
